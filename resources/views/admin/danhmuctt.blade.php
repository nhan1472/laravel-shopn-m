...
@section('content')
    <div class="mt-5" ng-controller="dmcontroller">
        ...
        <div ng-if="edit">
            <button ng-if="danhmuc" class="btn btn-primary" ng-click="them()">Thêm Tin tức</button>
            <table class="table" ng-if="danhmuc">
                ...
                <tbody>
                    <tr ng-repeat="item in listDmtin">
                        ...
                        <td ng-if="edit">
                            <form action="{{ route('admin.tintuc.xoadanhmuc') }}" method="post">
                                @csrf
            ...
            <div ng-if="!danhmuc">
                <form action="{{ route('admin.tintuc.adddanhmuc') }}" method="post">
                    @csrf
                    <table class="table">
                        <tbody>
                            <tr>
        ...
        <div ng-if="!edit">
            <form action="{{ route('admin.tintuc.danhmucedit') }}" method="post">
                @csrf
    ...
    <script>
        ...
        app.controller('dmcontroller', function($scope, $rootScope) {
            ...
            $scope.sua = function(i) {
                $scope.id = $scope.listDmtin[i].id
                $scope.name = $scope.listDmtin[i].name
            ...
            $scope.xoa = function(event) {
                if (!confirm(
                        'bạn có muốn xóa nó không\n nếu bạn xóa tất cả các Tin tức thuộc danh mục cũng sẽ xóa'
                    )) {
                ...
                @foreach ($listDmtin as $item)
                    {
                        ...
                        'name': '{{ $item->name }}',
                        @php
                            $sl = DB::select('SELECT COUNT(id) as sl FROM `tintuc` WHERE `idloai`=' . $item->id);
                        @endphp 'sl': {{ $sl[0]->sl }},
                    },
                @endforeach
            ]
